        ...
        try:
            ...
            if hasattr(self.config, 'min_price_threshold') and self.config.min_price_threshold > 0:
                if alpha_price < self.config.min_price_threshold:
                    ...
                    console.print(
                        f"⏸️  Price too low: {alpha_price:.6f} TAO < {self.config.min_price_threshold:.6f} TAO threshold")
                    console.print(
                        f"   💡 Waiting for higher price. Current: {alpha_price:.6f} TAO, Target: ≥{self.config.min_price_threshold:.6f} TAO")
            ...
            if hasattr(self.config, 'min_holdings_threshold'):
                ...
                if remaining_after_sale < self.config.min_holdings_threshold:
                    console.print(
                        f"🛑 Would leave holdings below threshold: {remaining_after_sale:.6f} < {self.config.min_holdings_threshold:.6f} alpha")
                    return True  # Skip this sale to maintain minimum holdings

            tao_to_earn = self.config.unstake_amount * alpha_price

            console.print(
                f"🔄 Attempting sale: {self.config.unstake_amount:.6f} alpha → {tao_to_earn:.6f} TAO @ {alpha_price:.6f} TAO/alpha")
...
def parse_args():
    ...
    parser.add_argument("--target_netuid", type=int, required=True,
                        help="The subnet you want to sell alpha from (should match your DCA bot)")
    parser.add_argument("--unstake_amount", type=float, required=True, help="Fixed alpha amount to sell each interval")
    parser.add_argument("--interval_seconds", type=int, default=10,
                        help="How often to check for selling opportunities (in seconds)")
    parser.add_argument("--min_price_threshold", type=float, default=0.08,
                        help="Only sell if alpha price is at or above this value (in TAO)")
    parser.add_argument("--min_holdings_threshold", type=float, default=0.5,
                        help="Never sell if it would leave you with less than this amount of alpha")
